...
Player::Player():k(1),num(0), sum(0)
    ...
    void Player::Kill(int i, int j)
    {
        ...
        signKill(i, j);
            ...
            if (board[i][j + 1] == -1 || board[i][j - 1] == -1)
            {
                ...
                board[i][j + k] = -2;
                if (i!=0 && i!=11 && j+k!=0 && j+k!=11)emit signBoner(i, j+k);
                k = 1;
                while (board[i][j - k] == -1)
                {
                    board[i][j - k] = -3;
            ...
            else if (board[i + 1][j] == -1 || board[i - 1][j] == -1)
            {
                ...
                board[i + k][j] = -2;
                if (i+k!=0 && i+k!=11 && j!=0 && j!=11) emit signBoner(i+k, j);
                k = 1;
                while (board[i - k][j] == -1)
                {
                    board[i - k][j] = -3;
            ...
            {
                board[i][j + 1] = -2;
                if (i!=0 && i!=11 && j+1!=0 && j+1!=11)emit signBoner(i, j+1);
                board[i][j - 1] = -2;
                if (i!=0 && i!=11 && j-1!=0 && j-1!=11)emit signBoner(i, j-1);
                board[i + 1][j] = -2;
                if (i+1!=0 && i+1!=11 && j!=0 && j!=11)emit signBoner(i+1, j);
                board[i - 1][j] = -2;
                if (i-1!=0 && i-1!=11 && j!=0 && j!=11)emit signBoner(i-1, j);
            }
            return;
    ...
    /*Cell Board::autoAttack(Level level)
        {
            Cell cell;
